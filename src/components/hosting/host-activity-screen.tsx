    ...
    <ScrollView ref={scroll} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      ...
      {step === 1 && <View style={{ marginTop: 34 }}><Text style={s.label}>ACCESS SETTINGS</Text><View style={{ marginTop: 22 }}><ChoiceRow label="Visibility" value={draft.visibility === 'public' ? 'Public' : draft.visibility === 'squad' ? 'Squad' : 'Private'} onPress={() => setDialog('visibility')} />
        ...
        <View style={{ marginTop: 24, gap: 12 }}>
          <Text style={s.label}>ACTIVITY PRICE</Text>
          <View accessibilityRole="radiogroup" style={s.priceSelector}>
            {([{ label: 'Free', paid: false }, { label: 'Paid', paid: true }] as const).map(option => <Pressable key={option.label} accessibilityRole="radio" accessibilityState={{ checked: draft.isPaid === option.paid }} aria-checked={draft.isPaid === option.paid} onPress={() => patch({ isPaid: option.paid, price: option.paid ? draft.price : '' })} style={[s.priceOption, draft.isPaid === option.paid && s.priceOptionSelected]}><Text style={[s.priceOptionText, draft.isPaid === option.paid && s.priceOptionTextSelected]}>{option.label}</Text></Pressable>)}
          </View>
          {draft.isPaid ? <><Text style={[s.body, { fontSize: 12 }]}>Activity Price</Text><Control label="Activity Price" icon="cash-outline" keyboardType="decimal-pad" inputMode="decimal" value={draft.price} maxLength={10} onChangeText={value => patch({ price: value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1') })} placeholder="₹ 250" /><Text style={s.small}>Participation cost payable to the organizer at the activity. WeNitro does not collect this payment.</Text></> : <Text style={s.small}>This Activity is free to join.</Text>}
        </View>
...
const createStyles = (c: HostColors) => StyleSheet.create({
  ...
  priceSelector: { flexDirection: 'row', padding: 4, gap: 4, borderWidth: 1, borderColor: c.border, borderRadius: 13, backgroundColor: c.input }, priceOption: { flex: 1, minHeight: 42, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, priceOptionSelected: { backgroundColor: c.purple }, priceOptionText: { color: c.muted, fontSize: 13, fontWeight: '700' }, priceOptionTextSelected: { color: '#FFFFFF' },
