...
export function CommunityConversation({ id, userId, name, avatar, success, onDismissSuccess, onBack, onInfo, onMessage }: { id: string; userId: string; name: string; avatar?: string; success: boolean; onDismissSuccess: () => void; onBack: () => void; onInfo: () => void; onMessage: (message: ChatMessage) => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setPostingAllowed(!community.adminsOnly || community.permissions.can_post);
  ...
  useEffect(() => {
    const channel = supabase.channel('community-metadata:' + id).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tbl_chat_rooms', filter: 'id=eq.' + id }, () => {
      void communitiesProductionService.getCommunity(id).then(community => { setMetadata({ name: community.name, avatar: community.imageUrl || undefined, memberCount: community.memberCount }); setPostingAllowed(!community.adminsOnly || community.permissions.can_post); }).catch(e => setError(errorText(e)));
    }).subscribe();
  ...
  return <SafeAreaView style={s.chat} edges={['top', 'bottom']}><KeyboardAvoidingView style={s.chat} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
    ...
    {loading ? <View style={s.center}><ActivityIndicator color="#8B81FF" /></View> : <ScrollView ref={scroll} keyboardShouldPersistTaps="handled" contentContainerStyle={[s.messages, !messages.length && { flexGrow: 1, justifyContent: 'flex-end' }]} onContentSizeChange={() => { if (historyScrollHold.current) { historyScrollHold.current = false; return; } if (!historyLoading) scroll.current?.scrollToEnd({ animated: false }); }}>
      ...
      {messages.map((message, index) => { const mine = String(message.sender_id) === userId; const day = new Date(message.created_at).toLocaleDateString(); const prev = index ? new Date(messages[index - 1].created_at).toLocaleDateString() : ''; const read = Object.entries(readAt).some(([uid, at]) => uid !== userId && at >= message.created_at); const media = message.media_signed_url; return <View key={message.id}>
        ...
        <View style={[s.messageRow, { alignItems: mine ? 'flex-end' : 'flex-start' }]}><View style={[s.bubble, mine && s.ownBubble, Boolean(message.poll_id || message.media_type === 'video' || message.share_payload) && { backgroundColor: message.poll_id ? c.card : mine ? c.accent : c.card, borderWidth: 2, borderColor: mine ? c.accent : c.border, minWidth: 225 }]}>{!mine ? <Text style={s.sender}>{message.profiles?.full_name || message.profiles?.username || 'Member'} <VerifiedBadge userId={message.sender_id} /></Text> : null}{message.deleted_at ? <Text style={s.muted}>Message deleted</Text> : <>{message.poll_id ? (polls[message.poll_id] ? <PollCard poll={polls[message.poll_id]} roomId={id} onUpdated={poll => setPolls(current => ({ ...current, [poll.id]: poll }))} /> : <Text style={s.muted}>Loading poll...</Text>) : null}{media && message.media_type === 'image' ? <Pressable accessibilityRole="button" accessibilityLabel="Open attached image" onPress={() => void Linking.openURL(media)}><Image source={{ uri: media }} style={s.messageImage} /></Pressable> : media ? <Pressable accessibilityRole="button" accessibilityLabel={message.media_type === 'video' ? 'Play Video' : 'Open attachment'} onPress={() => message.media_type === 'video' ? setPlayingVideo(media) : void Linking.openURL(media)}>{message.media_type === 'video' ? <View style={{ width: 220, height: 135, alignItems: 'center', justifyContent: 'center', gap: 14 }}><Icon name="film-outline" size={35} color={mine ? "#FFF" : c.text} /><Text style={[s.body, mine && s.ownText]}>Play Video</Text></View> : <Text style={[s.body, mine && s.ownText]}>Open {message.media_type || 'attachment'}</Text>}</Pressable> : null}{message.share_payload ? <Pressable accessibilityRole="button" accessibilityLabel={`Open shared ${message.share_payload.kind}: ${message.share_payload.title}`} onPress={() => openSharedContent(message.share_payload!)} style={s.shareCard}>{message.share_payload.thumbnailUrl ? <Image source={{ uri: message.share_payload.thumbnailUrl }} style={s.shareImage} /> : <View style={s.shareFallback}><Icon name="film-outline" size={27} color="#FFF" /></View>}<View style={s.shareCopy}><Text style={s.shareKind}>SHARED {message.share_payload.kind.replaceAll('_', ' ').toUpperCase()}</Text><Text numberOfLines={2} style={s.shareTitle}>{message.share_payload.title}</Text>{message.share_payload.preview ? <Text numberOfLines={2} style={s.sharePreview}>{message.share_payload.preview}</Text> : null}{message.share_payload.creatorName ? <Text numberOfLines={1} style={s.shareCreator}>Created by {message.share_payload.creatorName}</Text> : null}<Text numberOfLines={1} style={s.shareCreator}>Shared by {message.profiles?.full_name || message.profiles?.username || 'a WeNitro member'}</Text></View></Pressable> : null}{message.body && !message.poll_id && !message.share_payload ? <Text selectable style={[s.body, mine && s.ownText]}>{message.body}</Text> : null}</>}</View><View style={s.timestamp}><Text style={s.time}>{new Date(message.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>{mine ? <Icon name={read ? 'checkmark-done' : 'checkmark'} color={read ? c.accent : c.iconMuted} size={13} /> : null}</View></View>
      </View>; })}
    </ScrollView>}
...
const createStyles = (c: ReferencePalette) => StyleSheet.create({
  ...
  shareCard: { width: 232, borderRadius: 11, overflow: 'hidden', backgroundColor: '#101D31' },
  shareImage: { width: '100%', height: 118, backgroundColor: '#1B2434' },
  shareFallback: { width: '100%', height: 72, backgroundColor: '#282A51', alignItems: 'center', justifyContent: 'center' },
  shareCopy: { padding: 10, gap: 3 }, shareKind: { color: '#9FBFFF', fontSize: 9, fontWeight: '800', letterSpacing: .7 },
  shareTitle: { color: '#FFF', fontSize: 13, fontWeight: '800' }, sharePreview: { color: '#C5CDDA', fontSize: 10, lineHeight: 14 }, shareCreator: { color: '#8F9BAE', fontSize: 9, marginTop: 3 },
