...
export function ReferenceFeed({ data, setData, go, openActivity, openCommunity, openVibe, openProfile, refreshOnMount = true }: { data: AppData; setData: React.Dispatch<React.SetStateAction<AppData>>; go: (s: Screen) => void; openActivity: (id: string) => void; openCommunity?: (id: string) => void; openVibe?: (id: string) => void; openProfile?: (id: string) => void; refreshOnMount?: boolean }) {
  ...
  const carouselCardWidth = Math.min(Math.max(width - 28, 280), 402);
  const carouselStep = carouselCardWidth + 10;
  ...
  return <Page>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 14, paddingBottom: 28 }}>
      <ScrollView ref={homeCarousel} horizontal showsHorizontalScrollIndicator={false} snapToInterval={carouselStep} decelerationRate="fast" onMomentumScrollEnd={event => setHomeSlide(Math.max(0, Math.min(2, Math.round(event.nativeEvent.contentOffset.x / carouselStep))))} style={{ marginHorizontal: -14 }} contentContainerStyle={{ paddingHorizontal: 14, paddingTop: 14, gap: 10 }}>
        <Pressable accessibilityRole="button" accessibilityLabel="Create an activity" onPress={() => go('host')} style={{ width: carouselCardWidth, minHeight: 180, borderRadius: 20, overflow: 'hidden', backgroundColor: c.isDark ? '#21185B' : '#3327CB', padding: 18, justifyContent: 'space-between' }}><Image source={require('../../../assets/photos/bonfire.jpg')} style={{ position: 'absolute', right: 0, top: 0, width: '72%', height: '100%' }} /><LinearGradient pointerEvents="none" colors={['#281267', '#281267D9', '#28126715']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ position: 'absolute', inset: 0 }} /><View style={{ flexDirection: 'row', alignItems: 'flex-start' }}><View style={{ flex: 1, gap: 6 }}><Text style={{ color: '#FFF', fontSize: 18, fontWeight: '800', maxWidth: 190 }}>Host activities. Bring people together.</Text><Text style={{ color: '#D9D5FF', fontSize: 10, lineHeight: 15, maxWidth: 195 }}>Create a real activity, choose how people join, and meet your community.</Text></View></View><View style={{ alignSelf: 'flex-start', backgroundColor: '#FFF', borderRadius: 15, paddingHorizontal: 13, paddingVertical: 7 }}><Text style={{ color: '#3D2BD5', fontSize: 9, fontWeight: '800' }}>Create Activity  →</Text></View></Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="Open V-Nitro Store" onPress={() => go('shop')} style={{ width: carouselCardWidth, minHeight: 146, borderRadius: 20, overflow: 'hidden', backgroundColor: c.isDark ? '#351276' : '#5B32DC', padding: 18, justifyContent: 'space-between' }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><View style={{ gap: 6 }}><Text style={{ color: '#FFF', fontSize: 18, lineHeight: 22, fontWeight: '800' }}>Explore the V-Nitro Store</Text><Text style={{ color: '#E4DFFF', fontSize: 10 }}>See your current balance and store availability.</Text></View><Icon name="gift-outline" size={35} color="#F0D14E" /></View><View><Text style={{ color: '#FFF', fontSize: 20, fontWeight: '900' }}>{data.nitro}</Text><Text style={{ color: '#D9D5FF', fontSize: 9 }}>V-Nitro Points</Text></View></Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="Explore all activities" onPress={() => go('activities')} style={{ width: carouselCardWidth, minHeight: 146, borderRadius: 20, overflow: 'hidden', backgroundColor: c.isDark ? '#113E4C' : '#087A8C', padding: 18, justifyContent: 'space-between' }}><View style={{ flexDirection: 'row', alignItems: 'flex-start' }}><View style={{ flex: 1, gap: 6 }}><Text style={{ color: '#FFF', fontSize: 18, fontWeight: '800' }}>Find your next activity.</Text><Text style={{ color: '#D8FAFF', fontSize: 10, lineHeight: 15, maxWidth: 250 }}>Browse live activities, filter by what matters, and connect nearby.</Text></View><Icon name="compass-outline" size={43} color="#77F0E6" /></View><View style={{ alignSelf: 'flex-start', backgroundColor: '#FFF', borderRadius: 15, paddingHorizontal: 13, paddingVertical: 7 }}><Text style={{ color: '#087A8C', fontSize: 9, fontWeight: '800' }}>Explore Activities  →</Text></View></Pressable>
      </ScrollView>
      <View accessibilityLabel={`Carousel slide ${homeSlide + 1} of 3`} style={{ flexDirection: 'row', alignSelf: 'center', gap: 6, marginTop: 9 }}>{[0, 1, 2].map(index => <Pressable key={index} accessibilityRole="button" accessibilityLabel={`Show carousel slide ${index + 1}`} onPress={() => { setHomeSlide(index); homeCarousel.current?.scrollTo({ x: index * carouselStep, animated: true }); }} style={{ width: homeSlide === index ? 20 : 7, height: 7, borderRadius: 4, backgroundColor: homeSlide === index ? c.accent : c.border }} />)}</View>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Invite your friends" onPress={() => go('inviteSquad')} style={{ marginTop: 18, minHeight: 142, borderRadius: 22, padding: 18, overflow: 'hidden', backgroundColor: c.isDark ? '#24205A' : '#EFEDFF', borderWidth: 1.5, borderColor: c.isDark ? '#7770E8' : '#8B7AF0', flexDirection: 'row', alignItems: 'center', gap: 15 }}><LinearGradient pointerEvents="none" colors={['#6954E7', '#167C83']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ position: 'absolute', inset: 0 }} /><View style={{ width: 58, height: 58, borderRadius: 19, backgroundColor: '#FFFFFF24', alignItems: 'center', justifyContent: 'center' }}><Icon name="people" color="#FFFFFF" size={29} /></View><View style={{ flex: 1 }}><Text style={{ color: '#FFFFFF', fontSize: 17, fontWeight: '900' }}>Invite your friends</Text><Text style={{ color: '#E4F0FF', fontSize: 11, lineHeight: 16, marginTop: 5 }}>Share your real referral link and build your squad.</Text><Text style={{ color: '#FFFFFF', fontSize: 10, fontWeight: '800', marginTop: 8 }}>Invite now  →</Text></View></Pressable>
      {data.vibes.length ? <><SectionHeading title="Vibes from the community" action="See all vibes" onAction={() => go('vibes')} /><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 9 }}>{data.vibes.slice(0, 6).map(vibe => <Pressable accessibilityRole="button" accessibilityLabel={`Open vibe ${vibe.text || vibe.id}`} onPress={() => openVibe ? openVibe(vibe.id) : go('vibes')} key={vibe.id} style={{ width: 132, height: 92, borderRadius: 15, overflow: 'hidden', backgroundColor: c.inset }}>{vibe.mediaType !== 'video' && displayableMedia(vibe.mediaUrl) ? <Image source={{ uri: vibe.mediaUrl }} style={{ width: '100%', height: '100%' }} resizeMode="cover" /> : <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}><Icon name="film-outline" size={30} color={c.accent} /></View>}<View style={{ position: 'absolute', inset: 0, backgroundColor: '#00000026', alignItems: 'center', justifyContent: 'center' }}><View style={{ width: 29, height: 29, borderRadius: 15, backgroundColor: '#07111FAD', alignItems: 'center', justifyContent: 'center' }}><Icon name="play" size={12} color="#FFF" /></View></View></Pressable>)}</ScrollView></> : null}
      {activities.length ? <><SectionHeading title="More Activities" action="View all" onAction={() => go('activities')} /><View style={{ gap: 10 }}>{activities.slice(0, 4).map(a => <Pressable accessibilityRole="button" accessibilityLabel={`Open activity ${a.title}`} key={`more:${a.id}`} onPress={() => openActivity(a.id)} style={{ minHeight: 92, borderWidth: 1, borderColor: c.border, borderRadius: 16, overflow: 'hidden', backgroundColor: c.card, flexDirection: 'row' }}><Image source={typeof a.image === 'string' ? { uri: a.image } : a.image} style={{ width: 96, alignSelf: 'stretch', backgroundColor: c.inset }} resizeMode="cover" /><View style={{ flex: 1, padding: 11, gap: 5 }}><Text numberOfLines={2} style={{ color: c.text, fontSize: 12, lineHeight: 16, fontWeight: '800' }}>{a.title}</Text><View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}><Icon name="calendar-outline" color={c.accent} size={11} /><Text numberOfLines={1} style={{ color: c.muted, fontSize: 8.5, flex: 1 }}>{activityTime(a.startsAt)}</Text></View><View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}><Icon name="location-outline" color={c.muted} size={11} /><Text numberOfLines={1} style={{ color: c.muted, fontSize: 8.5, flex: 1 }}>{a.where || 'Location to be decided'}</Text></View></View><View style={{ justifyContent: 'center', paddingRight: 9 }}><Icon name="chevron-forward" color={c.iconMuted} size={17} /></View></Pressable>)}</View></> : null}
      <SectionHeading title="How It Works" />
      <View style={{ borderWidth: 1, borderColor: c.border, backgroundColor: c.card, borderRadius: 18, padding: 15, gap: 12 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 11 }}><View style={{ width: 42, height: 42, borderRadius: 14, backgroundColor: c.isDark ? '#2B285F' : '#EFECFF', alignItems: 'center', justifyContent: 'center' }}><Icon name="shield-checkmark-outline" color={c.accent} size={22} /></View><View style={{ flex: 1 }}><Text style={{ color: c.text, fontSize: 13, fontWeight: '800' }}>Meet safely and know the rules</Text><Text style={{ color: c.muted, fontSize: 9.5, lineHeight: 15, marginTop: 3 }}>Read how WeNitro handles privacy and the conditions for using the app.</Text></View></View><View style={{ flexDirection: 'row', gap: 9 }}><Pressable accessibilityRole="link" onPress={() => go('privacyPolicy')} style={{ flex: 1, minHeight: 40, borderRadius: 11, borderWidth: 1, borderColor: c.border, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: c.accent, fontSize: 10, fontWeight: '800' }}>Privacy Policy</Text></Pressable><Pressable accessibilityRole="link" onPress={() => go('terms')} style={{ flex: 1, minHeight: 40, borderRadius: 11, borderWidth: 1, borderColor: c.border, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: c.accent, fontSize: 10, fontWeight: '800' }}>Terms &amp; Conditions</Text></Pressable></View></View>
      <SectionHeading title="V-Nitro Points" action="View history" onAction={() => go('nitroHistory')} />
      <Pressable accessibilityRole="button" onPress={() => go('shop')} style={{ borderWidth: 1, borderColor: '#6650F550', backgroundColor: c.isDark ? '#171938' : '#F1EFFF', borderRadius: 18, padding: 15, flexDirection: 'row', alignItems: 'center', gap: 12 }}><Icon name="flash" color="#E6C84D" size={24} /><View style={{ flex: 1 }}><Text style={{ color: c.text, fontSize: 13, fontWeight: '800' }}>{data.nitro} V-Nitro Points</Text><Text style={{ color: c.muted, fontSize: 10, marginTop: 3 }}>Your current account balance. Store availability is shown inside.</Text></View><Icon name="chevron-forward" color={c.muted} /></Pressable>
    ...
    {filterOpen && <Sheet title="Filters" close={() => setFilterOpen(false)} footer={<View style={{ flexDirection: 'row', gap: 10 }}><View style={{ flex: 1 }}><Button label="Reset All" onPress={() => { setDraftCategories([]); setDraft({ dateFrom:'', dateTo:'', price:'All', gender:'All', verifiedOnly:false }); }} /></View><View style={{ flex: 1 }}><Button label="Apply Filters" onPress={() => { setCategories(draftCategories); setDateFrom(draft.dateFrom); setDateTo(draft.dateTo); setPrice(draft.price); setGender(draft.gender); setVerifiedOnly(draft.verifiedOnly); setFilterOpen(false); }} /></View></View>}>
      ...
      <Text style={{ color:c.text,fontWeight:'700',fontSize:14 }}>Activity costs</Text><Pills values={['All','Free','Costs may apply']} selected={draft.price} onChange={value=>setDraft(v=>({...v,price:value}))} />
