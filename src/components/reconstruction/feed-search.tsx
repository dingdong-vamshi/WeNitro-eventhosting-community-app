  ...
  return <Page>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 28 }}>
      ...
      <View style={{ paddingHorizontal: 14 }}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -14, marginTop: 14 }} contentContainerStyle={{ paddingHorizontal: 14, gap: 7 }}>
          {[['Trending', 'flame-outline'], ['Nearby', 'location-outline'], ['Today', 'time-outline'], ['Tomorrow', 'calendar-outline']].map(([value, icon]) => {
            const active = filter === value;
            return <Pressable accessibilityRole="button" accessibilityState={{ selected: active }} onPress={() => void chooseFilter(active ? 'All' : value)} key={value} style={{ minHeight: 40, paddingHorizontal: 14, borderRadius: 20, backgroundColor: active ? c.accent : c.card, borderWidth: 1, borderColor: active ? c.accent : c.border, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <Icon name={icon as any} size={15} color={active ? '#FFF' : c.accent} />
              <Text style={{ color: active ? '#FFF' : c.text, fontSize: 13, fontWeight: '800' }}>{value}</Text>
            </Pressable>;
          })}
          <Pressable accessibilityRole="button" accessibilityLabel="Open activity filters" onPress={() => { setDraftCategories(categories); setDraft({ dateFrom, dateTo, price, gender, verifiedOnly }); setFilterOpen(true); }} style={{ minHeight: 40, paddingHorizontal: 14, borderRadius: 20, backgroundColor: categories.length || dateFrom || dateTo || price !== 'All' || gender !== 'All' || verifiedOnly ? c.accent : c.card, borderWidth: 1, borderColor: categories.length || dateFrom || dateTo || price !== 'All' || gender !== 'All' || verifiedOnly ? c.accent : c.border, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Icon name="options-outline" size={15} color={categories.length || dateFrom || dateTo || price !== 'All' || gender !== 'All' || verifiedOnly ? '#FFF' : c.accent} />
            <Text style={{ color: categories.length || dateFrom || dateTo || price !== 'All' || gender !== 'All' || verifiedOnly ? '#FFF' : c.text, fontSize: 13, fontWeight: '800' }}>Filter</Text>
          </Pressable>
        </ScrollView>
        ...
        {loading ? <View style={{ height: 184, borderRadius: 17, backgroundColor: c.card, marginTop: 10 }} /> : <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -14, marginTop: 10 }} contentContainerStyle={{ paddingHorizontal: 14, gap: 10 }}>{activities.slice(0, 6).map(a => { const liked = data.likedIds.includes(`activity:${a.id}`); return <View key={a.id} style={{ width: 220, height: 198, borderRadius: 17, overflow: 'hidden', backgroundColor: c.card, borderWidth: 1, borderColor: c.border }}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel={`Open activity ${a.title} details`} onPress={() => openActivity(a.id)} style={({ pressed }) => ({ paddingHorizontal: 10, paddingVertical: 9, gap: 5, opacity: pressed ? .7 : 1 })}>
            ...
            <View style={{ flexDirection: 'row', gap: 5, alignItems: 'center' }}><Icon name="calendar-outline" color={c.accent} size={14} /><Text style={{ color: c.text, fontSize: 12, fontWeight: '700', flex: 1 }} numberOfLines={1}>{activityTime(a.startsAt)}</Text></View>
        ...
        <View style={{ marginTop: 18, minHeight: 142, borderRadius: 22, padding: 18, overflow: 'hidden', flexDirection: 'row', alignItems: 'center', gap: 15 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: '#E4F0FF', fontSize: 13, lineHeight: 18, fontWeight: '700', marginTop: 5 }}>Earn 10 Nitro Points.</Text>
            <Text style={{ color: '#E4F0FF', fontSize: 12, lineHeight: 17, marginTop: 4 }}>Share your real referral link and build your squad.</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14, marginTop: 10 }}>
              <Pressable accessibilityRole="button" accessibilityLabel="Invite now" onPress={() => go('inviteSquad')}><Text style={{ color: '#FFFFFF', fontSize: 13, fontWeight: '800' }}>Invite now  →</Text></Pressable>
              <Pressable accessibilityRole="button" accessibilityLabel="How it works" onPress={() => go('inviteSquad')}><Text style={{ color: '#FFFFFF', fontSize: 13, fontWeight: '800', textDecorationLine: 'underline' }}>How it works</Text></Pressable>
            </View>
          </View>
        </View>
        ...
        {activities.length ? <><SectionHeading title="More Activities" action="View all" onAction={() => go('activities')} /><View style={{ gap: 10 }}>{activities.slice(0, 4).map(a => <Pressable accessibilityRole="button" accessibilityLabel={`Open activity ${a.title}`} key={`more:${a.id}`} onPress={() => openActivity(a.id)} style={{ minHeight: 100, borderWidth: 1, borderColor: c.border, borderRadius: 16, overflow: 'hidden', backgroundColor: c.card, flexDirection: 'row' }}><Image source={typeof a.image === 'string' ? { uri: a.image } : a.image} style={{ width: 96, alignSelf: 'stretch', backgroundColor: c.inset }} resizeMode="cover" /><View style={{ flex: 1, padding: 11, gap: 6 }}><Text numberOfLines={2} style={{ color: c.text, fontSize: 14, lineHeight: 18, fontWeight: '800' }}>{a.title}</Text><View style={{ flexDirection: 'row', gap: 5, alignItems: 'center' }}><Icon name="calendar-outline" color={c.accent} size={14} /><Text numberOfLines={1} style={{ color: c.text, fontSize: 12, fontWeight: '700', flex: 1 }}>{activityTime(a.startsAt)}</Text></View><View style={{ flexDirection: 'row', gap: 5, alignItems: 'center' }}><Icon name="location-outline" color={c.muted} size={14} /><Text numberOfLines={1} style={{ color: c.muted, fontSize: 12, fontWeight: '600', flex: 1 }}>{a.where || 'Location to be decided'}</Text></View></View><View style={{ justifyContent: 'center', paddingRight: 9 }}><Icon name="chevron-forward" color={c.iconMuted} size={17} /></View></Pressable>)}</View></> : null}
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Open Nitro Store" onPress={() => go('shop')} style={{ marginTop: 18, minHeight: 142, borderRadius: 22, padding: 18, overflow: 'hidden', flexDirection: 'row', alignItems: 'center', gap: 15 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: '800', marginTop: 4 }}>{data.nitro} Nitro Points</Text>
            <Text style={{ color: '#E4F0FF', fontSize: 13, lineHeight: 18, fontWeight: '600', marginTop: 4 }}>Your current account balance. Store availability is shown inside.</Text>
            <Text style={{ color: '#FFFFFF', fontSize: 13, fontWeight: '800', marginTop: 10 }}>Open Store  →</Text>
          </View>
        </Pressable>
