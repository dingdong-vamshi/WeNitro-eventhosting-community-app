...
function ProfileLayout({ identity, summary, metrics, links, gallery, linksAvailable = true, owner = false, back, store, settings, editSocial, editProfile, verify, partnerAccount, partnerDashboard, contact, message, busy, squad, nitro, tab, setTab, onPhoto, children }: {
 identity: Identity; summary?: ProfileSummary; metrics: Metrics | null; links: SocialLinks; gallery: GalleryItem[]; linksAvailable?: boolean; owner?: boolean; back?: () => void; store?: () => void; settings?: () => void; editSocial?: () => void; editProfile?: () => void; verify?: () => void; partnerAccount?: { label: string; onPress: () => void }; partnerDashboard?: () => void; contact: () => void; message?: () => void; busy?: boolean; squad: () => void; nitro?: () => void; tab: Tab; setTab: (tab: Tab) => void; onPhoto?: (item: GalleryItem) => void; children: React.ReactNode;
}) {
 ...
 const values = [
  ...
  { label: 'Karma', value: rating == null ? null : Number(rating).toFixed(1), icon: 'star-outline' as const, color: '#D98737', action: () => setTab('Reviews') },
 ...
 return <Page>
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.body}>
   ...
   {summary && (summary.location || summary.about || summary.interests?.length || summary.communities?.length) ? <View style={[s.profileSummary, { backgroundColor: c.card, borderColor: c.border }]}>
    ...
    {summary.communities?.length ? <View style={{ gap: 7 }}><Text style={[s.summaryHeading, { color: c.text }]}>Communities</Text><View style={s.summaryChips}>{summary.communities.map(community => <View key={community} style={[s.summaryChip, { backgroundColor: c.inset }]}><Icon name="people-outline" size={14} color={c.accent} /><Text style={{ color: c.text, fontSize: 12, fontWeight: '600' }}>{community}</Text></View>)}</View></View> : null}
   </View> : null}
...
function TrustCard({ metrics, socialLinked, owner }: { metrics: Metrics | null; socialLinked?: boolean; owner: boolean }) {
 ...
 const displayedTotal = owner || metrics?.trust_score != null ? total : null;
...
function Content({ tab, loading, vibes, rows, reviews, openVibe, openActivity, openDraft }: { tab: Tab; loading: boolean; vibes: VibeReel[]; rows: Activity[]; reviews: ProfileReview[]; openVibe: (id: string) => void; openActivity: (a: Activity) => void; openDraft?: () => void }) {
 const c = usePalette();
...
export function ReferenceMemberProfile({ id, back, onConversation, onOpenActivity, onOpenVibe, onOpenSquad }: { id: string; back: () => void; onConversation: (id: string, person: any) => void; onOpenActivity: (activity: Activity) => void; onOpenVibe: (id: string) => void; onOpenSquad: (id: string) => void }) {
 ...
 useEffect(() => { let active = true; setLoading(true); setPerson(null); setError(''); void (async () => {
  ...
  const [p, m, a, v, squad, photos, joined, trust, reviews] = await Promise.all([
  ...
  if (!active) return;
  setPerson(p.data); setLinks(p.data?.social_links || {}); setHosted(items); setVibes(v.reels); setReviews(Array.isArray(reviews.data) ? reviews.data as ProfileReview[] : []);
  ...
  setGallery([{ uri: publicProfileImageUrl(p.data?.profile_image), position: 1 }, ...photos.map(photo => ({ uri: photo.public_url, position: photo.position }))].filter(item => item.uri).slice(0, 3));
