...
type Metrics = { verified?: boolean; nitro?: number | null; karma?: number | null; trust_score?: number | null; rating?: number | null; activities?: number | null; squad?: number | null; phone_verified?: boolean; aadhaar_verified?: boolean };
type Tab = 'My Vibes' | 'Upcoming' | 'Completed' | 'Drafts' | 'Reviews';
...
/** Both viewing modes share the reference structure; permissions only change actions/content. */
function ProfileLayout({ identity, metrics, links, linksAvailable = true, owner = false, back, store, settings, editSocial, editProfile, verify, contact, message, busy, squad, nitro, tab, setTab, children }: {
 identity: Identity; metrics: Metrics | null; links: SocialLinks; linksAvailable?: boolean; owner?: boolean; back?: () => void; store?: () => void; settings?: () => void; editSocial?: () => void; editProfile?: () => void; verify?: () => void; contact: () => void; message?: () => void; busy?: boolean; squad: () => void; nitro?: () => void; tab: Tab; setTab: (tab: Tab) => void; children: React.ReactNode;
}) {
 ...
 const values = [
  { label: 'Activities', value: metrics?.activities, icon: 'calendar-outline', color: '#9563D4', action: () => setTab('Upcoming') },
  { label: 'Squad', value: metrics?.squad, icon: 'people-outline', color: '#7769D0', action: squad },
  { label: 'V-Nitro', value: metrics?.nitro, icon: 'flash-outline', color: '#AA62D0', action: nitro },
  { label: 'Karma', value: metrics?.karma == null ? null : Number(metrics.karma).toFixed(1), icon: 'star-outline', color: '#D98737', action: () => setTab('Reviews') },
 ] as const;
 return <Page>
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.body}>
   <View style={s.identity}>
    {identity.avatar ? <Image source={{ uri: identity.avatar }} style={s.avatar} /> : <LinearGradient colors={['#FFA350', '#DB6B3D']} style={[s.avatar, s.center]}><Text style={s.initials}>{initials}</Text></LinearGradient>}
    <View style={{ flex: 1, gap: 5 }}><View style={s.nameRow}><Text numberOfLines={2} style={[s.name, { color: c.text }]}>{identity.name || identity.username}</Text><VerifiedBadge userId={identity.id} size={17} /></View>{identity.bio ? <Text numberOfLines={2} style={[s.secondary, { color: c.muted }]}>{identity.bio}</Text> : null}</View>
   </View>
...
function TrustCard({ metrics, socialLinked }: { metrics: Metrics | null; socialLinked?: boolean }) {
 ...
 const checks = [
  { label: 'Phone Verified', done: metrics?.phone_verified, note: metrics?.phone_verified === undefined ? 'Not shared' : 'Pending' },
  { label: 'Aadhaar Verified', done: metrics?.aadhaar_verified, note: 'Unavailable' },
  { label: 'Social Linked', done: socialLinked, note: socialLinked === undefined ? 'Not shared' : 'Not added' },
  { label: 'Activities Join / Host', done: (metrics?.activities ?? 0) > 0, note: 'Pending' },
  { label: '4+ Avg Rating', done: (metrics?.rating ?? 0) >= 4, note: 'Pending' },
 ];
 return <View style={[s.trust, { backgroundColor: c.card, borderColor: c.border }]}><View style={s.nameRow}><Icon name="shield-checkmark-outline" color={c.accent} size={19} /><Text style={[s.sectionTitle, { color: c.text }]}>Trust Score</Text></View><Text style={{ color: c.muted, fontSize: 10, marginTop: 3, marginLeft: 25 }}>{metrics?.verified ? 'Verified Profile' : 'Standard Profile'}</Text><View style={s.trustBody}><View style={[s.scoreRing, { borderColor: c.isDark ? '#363248' : '#E9E5F6' }]}><Text style={[s.score, { color: c.text }]}>{score ?? '—'}</Text><Text style={{ color: c.muted, fontSize: 10 }}>/ 100</Text></View><View style={{ flex: 1, gap: 8 }}>{checks.map(check => <View key={check.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}><Icon name={check.done ? 'checkmark-circle' : 'ellipse-outline'} size={13} color={check.done ? '#22A377' : c.iconMuted} /><Text style={{ flex: 1, color: check.done ? c.text : c.muted, fontSize: 10 }}>{check.label}</Text>{!check.done && <Text style={{ color: c.iconMuted, fontSize: 8 }}>{check.note}</Text>}</View>)}</View></View>{score == null && <Text style={{ color: c.muted, fontSize: 9, marginTop: 8 }}>A numeric trust score is not available yet.</Text>}</View>;
}
...
function Content({ tab, loading, vibes, rows, openVibe, openActivity, openDraft }: { tab: Tab; loading: boolean; vibes: VibeReel[]; rows: Activity[]; openVibe: (id: string) => void; openActivity: (a: Activity) => void; openDraft?: () => void }) {
 ...
 if (tab === 'Reviews') return <Empty icon="chatbox-ellipses-outline" text="No public reviews are available yet." />;
...
export function ReferenceProfile({ data, setData, go, openActivity, openDraft, openVibe, openSquad }: { data: AppData; setData: React.Dispatch<React.SetStateAction<AppData>>; go: (screen: Screen) => void; openActivity: (id: string) => void; openDraft: () => void; openVibe: (id: string) => void; openSquad: () => void }) {
 ...
 const [tab, setTab] = useState<Tab>('My Vibes'), [error, setError] = useState(''), [loading, setLoading] = useState(true), [vibes, setVibes] = useState<VibeReel[]>([]), [activities, setActivities] = useState<Activity[]>([]), [cursor, setCursor] = useState<string | null>(null), [contact, setContact] = useState<{ email?: string; phone?: string } | null>(null);
 const request = useRef(0);
 useEffect(() => { let active = true; void Promise.all([supabase.rpc('my_profile_metrics'), supabase.rpc('my_social_links')]).then(([m, l]) => { if (!active) return; if (m.error || l.error) { setError(m.error?.message || l.error?.message || 'Could not load profile.'); return; } setMetrics(m.data); setLinks(l.data || {}); }); return () => { active = false; }; }, [data.userId]);
 ...
 const rows = activities.filter(a => tab === 'Drafts' ? a.status === 'draft' : tab === 'Completed' ? completed(a) : !completed(a));
 ...
 return <><ProfileLayout identity={{ id: data.userId!, name: data.name, username: data.username, avatar: data.avatarUri, bio: data.bio }} metrics={metrics} links={links} owner store={() => go('shop')} settings={() => go('settings')} editSocial={() => go('socialLinks')} editProfile={() => go('editProfile')} verify={() => go('verification')} contact={openContact} squad={openSquad} nitro={() => go('nitroHistory')} tab={tab} setTab={setTab}><ErrorLine text={error} /><Content tab={tab} loading={loading} vibes={vibes} rows={rows} openVibe={openVibe} openActivity={a => openActivity(a.id)} openDraft={openDraft} />{cursor && tab !== 'Reviews' && <View style={{ padding: 10 }}><Button label="Load more" busy={loading} onPress={() => void load(true)} /></View>}</ProfileLayout>{contact && <ContactSheet contact={contact} close={() => setContact(null)} />}</>;
}
...
export function ReferenceMemberProfile({ id, back, onConversation, onOpenActivity, onOpenVibe, onOpenSquad }: { id: string; back: () => void; onConversation: (id: string, person: any) => void; onOpenActivity: (activity: Activity) => void; onOpenVibe: (id: string) => void; onOpenSquad: (id: string) => void }) {
 ...
 useEffect(() => { let active = true; setLoading(true); setPerson(null); setError(''); void (async () => {
  ...
  setMetrics({ verified: p.data?.is_verified === true || m.data?.isverified === 1, activities: a.total ?? items.length, squad: squad.error ? null : squad.count, nitro: m.data?.points == null ? null : Number(m.data.points), karma: null, rating: m.data?.rating == null ? null : Number(m.data.rating), trust_score: null });
 ...
 return <><ProfileLayout identity={{ id, name: person.fullname, username: person.username || '', avatar: person.profile_image, bio: person.bio }} metrics={metrics} links={links} linksAvailable={person.social_links != null} back={back} contact={() => setContact(true)} message={person.can_message ? message : undefined} busy={busy} squad={() => onOpenSquad(id)} tab={tab} setTab={setTab}><ErrorLine text={error} /><Content tab={tab} loading={loading} vibes={vibes} rows={hosted.filter(a => tab === 'Completed' ? completed(a) : !completed(a))} openVibe={onOpenVibe} openActivity={onOpenActivity} /></ProfileLayout>{contact && <ContactSheet contact={person} close={() => setContact(false)} />}</>;
}
...
const s = StyleSheet.create({
 ...
 metrics: { flexDirection: 'row', gap: 7 }, metric: { height: 91, borderRadius: 12, borderWidth: 1, alignItems: 'center', justifyContent: 'center', gap: 4 }, metricIcon: { width: 28, height: 28, borderRadius: 9, alignItems: 'center', justifyContent: 'center' }, metricValue: { fontSize: 18, fontWeight: '800' }, metricLabel: { fontSize: 8, fontWeight: '700', letterSpacing: .3 },
