...
export function ReferenceVerification({ back }: { back: () => void }) {
  ...
  const load = async () => {
    ...
    const fallbackScore = derivedTrustScore({
      ...
      aadhaar_verified: Boolean(combinedMetrics.aadhaar_verified),
      social_linked: Boolean(combinedMetrics.social_linked),
      rating: Number(combinedMetrics.rating ?? 0),
  ...
  return <Page><Header title="Verify Your Account" back={back} />{loading ? <Skeleton /> : <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 18, gap: 18, paddingBottom: 40 }}>
    ...
    <View style={{ backgroundColor: c.card, borderRadius: 16, padding: 17, borderWidth: 1, borderColor: c.border, gap: 13 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><Icon name={photoVerified ? 'checkmark-circle' : 'camera-outline'} color={photoVerified ? '#2D9666' : c.accent} /><View style={{ flex: 1 }}><Text style={{ color: c.text, fontSize: 15, fontWeight: '700' }}>{photoVerified ? 'Selfie Uploaded' : 'Upload Selfie'}</Text><Text style={{ color: c.muted, fontSize: 12, lineHeight: 18, marginTop: 4 }}>{photoVerified ? '+10 Trust / Nitro Points awarded once.' : 'Take a selfie with the camera · +10 points'}</Text></View><Text style={{ color: photoVerified ? '#198457' : c.muted, fontSize: 12, fontWeight: '700' }}>+10</Text></View>
      ...
      {photoVerified && <Text style={{ color: c.muted, fontSize: 12 }}>Replacing your selfie does not award additional points.</Text>}
    </View>
...
export function ReferenceEmergencyContact({ back }: { back: () => void }) {
  ...
  return <Page><Header title="Emergency Contact" back={back} />{loading ? <Skeleton /> : <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 18, gap: 17 }}><View style={{ backgroundColor: c.isDark ? '#38232B' : '#FFF0F1', borderWidth: 1, borderColor: c.isDark ? '#72404D' : '#F4B8BF', borderRadius: 14, padding: 14, flexDirection: 'row', gap: 11 }}><Icon name="warning-outline" color={c.isDark ? '#FF8A99' : '#C93D51'} /><View style={{ flex: 1, gap: 4 }}><Text style={{ color: c.isDark ? '#FFA0AD' : '#A52237', fontWeight: '700', fontSize: 13 }}>SOS Quick Contact</Text><Text style={{ color: c.isDark ? '#D5A4AC' : '#8D3D48', fontSize: 12, lineHeight: 18 }}>This trusted contact may be used for safety escalation. Keep the number current.</Text></View></View>
...
export function ReferenceNitroHistory({ back }: { back: () => void }) {
  ...
  return <Page><Header title="Nitro Points History" back={back} />{loading ? <Skeleton /> : <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}><View style={{ borderRadius: 17, padding: 18, backgroundColor: c.card, borderWidth: 1, borderColor: c.border, flexDirection: 'row', alignItems: 'center', gap: 12 }}><Icon name="flash" color="#C0A72B" size={29} /><View style={{ flex: 1, gap: 5 }}><Text style={{ color: c.text, fontSize: 15, fontWeight: '700' }}>Current Nitro Points Balance</Text><Text style={{ color: c.muted, fontSize: 12 }}>{balance} Nitro Points</Text></View></View><Text style={{ color: c.text, fontSize: 17, fontWeight: '700' }}>Point Ledger</Text><ErrorLine text={error} />{rows.map(row => <View key={row.id} style={{ backgroundColor: c.card, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: c.border, flexDirection: 'row', alignItems: 'center', gap: 12 }}><View style={{ width: 39, height: 39, borderRadius: 20, backgroundColor: row.points >= 0 ? '#DFF6E9' : '#FDE5E8', alignItems: 'center', justifyContent: 'center' }}><Icon name="flash" color={row.points >= 0 ? '#2D9666' : '#C93D51'} size={19} /></View><View style={{ flex: 1, gap: 4 }}><Text style={{ color: c.text, fontSize: 13, fontWeight: '600' }}>{row.description}</Text><Text style={{ color: c.muted, fontSize: 12 }}>{new Date(row.created_at).toLocaleString()}</Text></View><Text style={{ color: row.points >= 0 ? '#2D9666' : '#C93D51', fontWeight: '800' }}>{row.points >= 0 ? '+' : ''}{row.points}</Text></View>)}{!rows.length && !error && <View style={{ alignItems: 'center', paddingTop: 55, gap: 12 }}><Icon name="receipt-outline" color={c.muted} size={42} /><Text style={{ color: c.text, fontWeight: '700' }}>No transactions yet</Text><Text style={{ color: c.muted, fontSize: 12, textAlign: 'center', lineHeight: 18 }}>Real Nitro Point entries will appear here when the account ledger changes.</Text></View>}</ScrollView>}</Page>;
}
