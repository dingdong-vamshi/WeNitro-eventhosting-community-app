...
export function ReferenceVerification({ back }: { back: () => void }) {
  ...
  const load = async () => {
    ...
    const [metrics, auth] = await Promise.all([supabase.rpc('my_profile_metrics'), supabase.auth.getUser()]);
...
export function ReferenceNitroHistory({ back }: { back: () => void }) {
  ...
  return <Page><Header title="V-Nitro History" back={back} />{loading ? <Skeleton /> : <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}><View style={{ borderRadius: 17, padding: 18, backgroundColor: c.card, borderWidth: 1, borderColor: c.border, flexDirection: 'row', alignItems: 'center', gap: 12 }}><Icon name="flash" color="#C0A72B" size={29} /><View style={{ flex: 1, gap: 5 }}><Text style={{ color: c.text, fontSize: 15, fontWeight: '700' }}>Current V-Nitro Balance</Text><Text style={{ color: c.muted, fontSize: 11 }}>{balance} V-Nitro Points</Text></View></View><Text style={{ color: c.text, fontSize: 17, fontWeight: '700' }}>Point Ledger</Text><ErrorLine text={error} />{rows.map(row => <View key={row.id} style={{ backgroundColor: c.card, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: c.border, flexDirection: 'row', alignItems: 'center', gap: 12 }}><View style={{ width: 39, height: 39, borderRadius: 20, backgroundColor: row.points >= 0 ? '#DFF6E9' : '#FDE5E8', alignItems: 'center', justifyContent: 'center' }}><Icon name="flash" color={row.points >= 0 ? '#2D9666' : '#C93D51'} size={19} /></View><View style={{ flex: 1, gap: 4 }}><Text style={{ color: c.text, fontSize: 12, fontWeight: '600' }}>{row.description}</Text><Text style={{ color: c.muted, fontSize: 9 }}>{new Date(row.created_at).toLocaleString()}</Text></View><Text style={{ color: row.points >= 0 ? '#2D9666' : '#C93D51', fontWeight: '800' }}>{row.points >= 0 ? '+' : ''}{row.points}</Text></View>)}{!rows.length && !error && <View style={{ alignItems: 'center', paddingTop: 55, gap: 12 }}><Icon name="receipt-outline" color={c.muted} size={42} /><Text style={{ color: c.text, fontWeight: '700' }}>No transactions yet</Text><Text style={{ color: c.muted, fontSize: 11, textAlign: 'center', lineHeight: 17 }}>Real V-Nitro entries will appear here when the account ledger changes.</Text></View>}</ScrollView>}</Page>;
}
