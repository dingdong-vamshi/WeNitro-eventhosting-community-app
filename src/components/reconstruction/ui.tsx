...
export function Button({ label, onPress, disabled, busy, danger = false, variant = 'solid' }: { label: string; onPress: () => void; disabled?: boolean; busy?: boolean; danger?: boolean; variant?: 'solid' | 'outline' }) { const outline = variant === 'outline'; return <Pressable accessibilityRole="button" disabled={disabled || busy} onPress={onPress} style={{ minHeight: 48, alignItems: 'center', justifyContent: 'center', backgroundColor: outline ? 'transparent' : danger ? '#D83C57' : purple, borderWidth: outline ? 1 : 0, borderColor: danger ? '#D83C57' : purple, borderRadius: 14, opacity: disabled || busy ? .5 : 1, padding: 12 }}>{busy ? <ActivityIndicator color={outline ? purple : 'white'} /> : <Text style={{ color: outline ? danger ? '#D83C57' : purple : 'white', fontSize: 14, fontWeight: '700' }}>{label}</Text>}</Pressable>; }
