...
export function BrandBar({ go, location = 'Bhubaneswar' }: { go?: (screen: any) => void; location?: string }) { const c = usePalette(); return <LinearGradient colors={c.isDark ? ['#2410B8', '#4937F0'] : ['#3024D8', '#6651F4']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={ui.brandBar}><View style={ui.brandIdentity}><ImageLogo /><View><Text style={ui.brandName}>WeNitro</Text><Text style={ui.brandLocation}><Ionicons name="location" size={9} /> {location}</Text></View></View><View style={{ flexDirection: 'row' }}><Action color="#FFFFFF" name="search-outline" label="Search" onPress={() => go?.('search')} /><Action color="#FFFFFF" name="notifications-outline" label="Notifications" onPress={() => go?.('notifications')} /></View></LinearGradient>; }
