...
export function ReferenceEditProfile({ data, setData, back, onSocialProfiles }: { data: AppData; setData: React.Dispatch<React.SetStateAction<AppData>>; back: () => void; onSocialProfiles?: () => void }) {
 const c = usePalette();
 ...
 return <Page><Header title="Edit Profile" back={() => { if (!busy) back(); }} /><KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 18, paddingBottom: 36, gap: 19 }}><ErrorLine text={error} />{!values ? !error && <Skeleton /> : <>
   <View style={{ alignItems: 'center', gap: 9 }}>{avatar ? <Image source={{ uri: avatar }} style={{ width: 72, height: 72, borderRadius: 36 }} /> : <Icon name="person-circle-outline" size={72} color={c.muted} />}<Pressable accessibilityRole="button" disabled={busy || photoBusy !== null} onPress={() => setPhotosOpen(true)}><Text style={{ color: purple, fontSize: 13 }}>Edit Photo</Text></Pressable><Text style={{ color: c.muted, fontSize: 10 }}>Manage up to 3 profile photos. Set one as primary.</Text>{pendingAvatar && <Text style={{ color: c.muted, fontSize: 11 }}>Photo uploads when you save.</Text>}</View>
  {onSocialProfiles && <Pressable accessibilityRole="button" disabled={busy} onPress={onSocialProfiles} style={{ minHeight: 46, flexDirection: 'row', alignItems: 'center', gap: 9, borderWidth: 1, borderColor: c.border, borderRadius: 11, paddingHorizontal: 12, backgroundColor: c.card }}><Icon name="share-social-outline" size={19} color={c.accent} /><Text style={{ flex: 1, color: c.text, fontSize: 13, fontWeight: '600' }}>Social Profiles</Text><Icon name="chevron-forward" size={17} color={c.muted} /></Pressable>}
 ...
 {photosOpen && <Sheet title="Manage Photos" close={() => { if (photoBusy === null) setPhotosOpen(false); }}><Text style={{ color: c.muted, fontSize: 11, lineHeight: 17 }}>Add, replace, or remove up to 3 photos. Set any photo as primary. Others can view these photos on your profile.</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>{Array.from({ length: 3 }, (_, index) => { const position=index+1; const photo=position===1 ? avatar ? { public_url: avatar } : null : photos.find(item => item.position===position); return <View key={position} style={{ width: '30.8%', aspectRatio: 1, borderRadius: 12, borderWidth: 1, borderStyle: photo ? 'solid' : 'dashed', borderColor: c.border, overflow: 'hidden', backgroundColor: c.card }}><Pressable accessibilityRole="button" accessibilityLabel={`${photo ? 'Replace' : 'Add'} photo ${position}`} disabled={photoBusy !== null} onPress={() => void choosePhoto(position)} style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>{photo ? <Image source={{ uri: photo.public_url }} style={{ width: '100%', height: '100%' }} /> : <><Icon name="add" color={purple} /><Text style={{ color: c.muted, fontSize: 9 }}>Photo {position}</Text></>}{photoBusy===position && <View style={{ position:'absolute', inset:0, backgroundColor:'#0008', alignItems:'center', justifyContent:'center' } as any}><Text style={{ color:'#FFF', fontSize:9 }}>Uploading...</Text></View>}</Pressable>{photo && photoBusy===null && <Pressable accessibilityRole="button" accessibilityLabel={`Remove photo ${position}`} onPress={() => void removePhoto(position)} style={{ position:'absolute', right:4, top:4, width:24, height:24, borderRadius:12, backgroundColor:'#C6384E', alignItems:'center', justifyContent:'center' }}><Icon name="close" color="#FFF" size={14} /></Pressable>}{photo && position !== 1 && photoBusy===null ? <Pressable accessibilityRole="button" accessibilityLabel={`Set photo ${position} as primary`} onPress={() => void (async () => { setPhotoBusy(position); setError(''); try { const url = await referenceDeltaService.setPrimaryProfilePhoto(position, photo.public_url, avatar); setAvatar(url); setPendingAvatar(null); setData(current => ({ ...current, avatarUri: url })); setPhotos(await referenceDeltaService.listProfilePhotos()); } catch (e) { setError(message(e)); } finally { setPhotoBusy(null); } })()} style={{ position:'absolute', left:4, bottom:4, backgroundColor:'#4F3BEE', borderRadius:8, paddingHorizontal:6, paddingVertical:3 }}><Text style={{ color:'#FFF', fontSize:8, fontWeight:'700' }}>Primary</Text></Pressable> : null}</View>; })}</View><ErrorLine text={error} /></Sheet>}
 </Page>;
}
