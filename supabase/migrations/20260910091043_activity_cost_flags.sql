-- Independent informational flags. Keep numeric payment and Partner guards intact.
alter table public.tbl_events add column costs_may_apply boolean not null default false, add column entry_fee_required boolean not null default false;
...
begin
  ...
  insert into public.tbl_events(
    ...
    longitude, verified_only, age_min, age_max, gender_preference, location_instruction, costs_may_apply, entry_fee_required, is_paid, price, currency, intent, status, media
  )
  values(
    ...
    coalesce((p_payload->>'costs_may_apply')::boolean, false),
    coalesce((p_payload->>'entry_fee_required')::boolean, false),
