...
const communityPolicyAccess = read('supabase/migrations/20260923052909_community_request_policy_helper_access.sql');
const partnerHistory = read('supabase/migrations/20260923060455_partner_application_history_admin_read.sql');
...
includesAll(settings, ['NitroBot Help', 'Ask NitroBot', 'nitroBotAnswer', 'It does not access private account data', "go('feedback')"]);
assert.ok(!settings.includes('NitroBot is not connected yet'), 'NitroBot must not remain a dead placeholder.');
includesAll(profile, ['Set as primary', 'Change photo', 'removePrimaryProfilePhoto', "showTab('Reviews')", "go('activityHistory')", "'Communities'"]);
includesAll(profile, ['profile_reviews', 'rater_avatar', 'event_title']);
const publicProfile = profile.slice(profile.indexOf('export function ReferenceMemberProfile'));
...
includesAll(communityPolicyAccess, ['grant execute on function private.community_can_approve(integer) to authenticated']);
includesAll(partnerHistory, [
  'admin_list_partner_application_history',
  'not public.is_wenitro_admin()',
