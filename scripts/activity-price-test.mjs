...
assert.match(host, /label: 'Free', paid: false/);
assert.match(host, /label: 'Paid', paid: true/);
assert.match(host, /label="Activity Price"/);
assert.match(host, /priceInr: draft\.isPaid \? Number\(draft\.price\) : 0/);
assert.match(app, /Activity Price \$\{activity\.price\}/);
assert.match(app, /Participation cost payable to the organizer at the activity/);
assert.match(app, /"Free Activity"/);
...
console.log('PASS: Free/Paid UI, onsite-only detail copy, card price badge, direct/approval SQL, and Cashfree isolation verified.');
